<div class="before-pdf-header text-light text-center py-3 mb-3">As a tempcover member enjoy a discount on policies
    lasting
    one day or more</div>
<div class="text-light text-center py-3 mb-3">
    <a href="https://www.tempcover.com/start-quote" target="_blank" class="btn-get-quote text-decoration-none">GET A QUOTE</a>
</div>

<h2 class="upm-author my-5">Hello <?php echo esc_html($current_user->display_name) ?></h2>
<h4 class="policy-header">My Tempcover policies</h4>