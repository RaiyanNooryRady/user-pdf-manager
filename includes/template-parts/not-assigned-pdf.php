<div class="upm-not-assigned-pdf my-3">
    <div class="card shadow-sm">
        <h5 class="policy-header">Current & Upcoming</h5>
        <div class="no-policy text-center py-5 rounded-4">
            <img src="<?php echo plugins_url() . '/user-pdf-manager/assets/images/no-policy.svg'; ?>"
                class="img-no-policy" alt="no-policy">

            <h4 class="text-light">You have no active temporary policies</h4>
            <div class="text-light text-center py-3">
                <a href="https://www.tempcover.com/start-quote" target="_blank" class="p-3 btn-get-quote text-decoration-none">Get a quote</a>
            </div>
        </div>
        <h5 class="policy-header">Past policies</h5>
    </div>

</div>
<?php //echo plugin_dir_url(__FILE__) . 'assets/images/no-policy.svg'; ?>